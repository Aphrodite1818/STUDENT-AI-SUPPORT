        ...
        <div>
          ...
          <nav className="p-4 space-y-2">
            {[
              ...
              { name: 'Teachers', icon: 'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z' },
              { name: 'Settings', icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z' },
            ].map((item, index) => (
              <a 
                key={index}
                href="#"
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${index === 0 ? 'bg-primary/10 text-primary' : 'text-text-soft hover:bg-surface-hover hover:text-text'}`}
                title={!isSidebarOpen ? item.name : undefined}
              >
                <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  {/* Depending on the path, some icons might need two paths, simplifying for the mockup */}
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.icon} />
                  {index === 3 && <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />}
                </svg>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-background relative overflow-y-auto">
        ...
        <header className="h-16 flex items-center justify-between px-8 border-b border-border/50 sticky top-0 bg-background/80 backdrop-blur-md z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button variant="outline" size="sm">Invite Teachers</Button>
          </div>
        </header>
